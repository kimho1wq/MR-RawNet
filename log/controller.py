...
class LogModuleController(ExperimentLogger):
    ...
    def log_metric(self, name, value, log_step=None, epoch_step=None):
        for component in self.components:
            component.log_metric(name, value, log_step, epoch_step)
    ...
    class Builder():
        def __init__(self, name, project):
            ...
            self.args['flag_use_neptune'] = False
            self.args['tags'] = []
        ...
        def use_neptune(self, user, api_token):
            self.args['flag_use_neptune'] = True
            self.args['neptune_user'] = user
            self.args['neptune_api_token'] = api_token
            return self
        ...
        def use_wandb(self, group, entity, api_key):
            self.args['flag_use_wandb'] = True
            self.args['group'] = group
            self.args['entity'] = entity
            self.args['wandb_api_key'] = api_key
            return self

        def build(self):
            modules = []

            if self.args['flag_use_neptune']:
                neptune = NeptuneLogger(
                    self.args['neptune_user'], 
                    self.args['neptune_api_token'], 
                    self.args['name'], 
                    self.args['project'], 
                    self.args['tags'], 
                    self.args['description'],
